...
import type { ConfigureRequest, ReadPinRequest, ReadPinResponse, SetPinRequest, SubscribePinRequest, SubscribedPinValue } from "./types";
...
io.on("configurePin", (_: Socket, data: ConfigureRequest) => {
    for (const pin of data.pins) {
        ...
        const option = pin.pull === "off" ? rpio.PULL_OFF : pin.pull === "down" ? rpio.PULL_DOWN : pin.pull === "up" ? rpio.PULL_UP : pin.initialValue === true ? rpio.HIGH : pin.initialValue === false ? rpio.LOW : undefined
...
io.on("readPin", (_: Socket, data: ReadPinRequest, cb: (d: ReadPinResponse) => void) => {
